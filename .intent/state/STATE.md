...
- `SIDEBAR-007`: Every required host source change is attributable to a selected realization, bound to a checked target baseline, and removable without erasing unrelated later changes; no second implementation silently shares ownership of the same target effect.
...
- The current refactor has no sealed realization lock. Its direct external-consumer registration fixture and source remain candidate evidence until committed, bound to a target and checked through the lifecycle above.
