...
Status: draft embedded intent package with candidate 11 selected for the local Linux Web workbench. Historical locks remain immutable evidence. No realization is active or user-accepted.
...
- `SIDEBAR-005`: Each session retains its own ordered instances and active selection across mounted-session changes without a second state mirror. Switching sessions preserves the Host-owned open state as well as width and maximization.
