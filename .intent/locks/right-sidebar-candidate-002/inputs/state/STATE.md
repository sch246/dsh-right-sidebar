...
Status: draft embedded intent package with one selected C2 candidate realization. Retained C1 is historical implementation and lifecycle evidence, not current acceptance.
...
- Full browser acceptance, production package/module loading, layout behavior, and main-view/sidebar session synchronization remain incomplete. Component tests cover empty, loading, failure/retry, focus repair and keyboard tab navigation; they do not replace browser evidence.
