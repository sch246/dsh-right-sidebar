...
Status: draft embedded intent package with a locally installed grouped-workbench candidate selected in `STATE.json`. [Deployment evidence](../logs/2026-09-05-grouped-workbench-deployment.md) distinguishes implementation, activation and bounded verification. Historical locks remain immutable evidence. No realization is active or user-accepted.
...
- `SIDEBAR-013`: Dropping a tab in a group's outer ten percent creates a half-area split on the selected edge; the center joins that group and tab-bar drops choose insertion order. Hover shows the resulting area without changing layout. Internal tab drags never enter feature-editor text or file drop handlers, while external editor drops remain untouched. Split dividers resize freely, remain recoverable at extremes and have no fixed maximum.
