...
- 组不显示焦点边框；`activeGroupId` 记录下次打开的目标组。切换目标组保留内容区的输入焦点，不强制聚焦标签。
...
每个组独立维护内存中的后退/前进历史。已提交的标签切换自动记录；功能在异步解析前调用 `beginNavigation()`，将其取消信号贯穿读取，并通过 `navigation.commit(instanceId, destination, apply)` 一次提交准备好的内容、描述符和历史。相同 opening request 对象共享一次导航。`onNavigate(descriptor, navigation)` 通过提供的导航对象回放，拒绝或取消时保留原内容和游标；成功回放不再次压栈。`recordNavigation()` 用于恢复后的初始记录，后台 checkpoint 使用 `updateInstance()`。关闭或移出组的目标被跳过，历史不随刷新持久化。此接口要求 sidebar >=0.0.5 <0.1.0。

标签栏不显示后退/前进按钮；Alt+左/右和鼠标后/前侧键作用于事件所在组，包括历史边界。导航期间用稳定的组容器承接即将离开的内容焦点，保留已进入目标编辑器的焦点。开始后发生任何键盘、指针、焦点或窗口失焦事件，迟到结果都不再转移焦点。普通控件操作、组间及聊天焦点切换不创建历史。

成功提交目的地会统一展开侧边栏并激活目标组和标签，复用已打开文件及历史回放也遵循此规则；新目的地按请求定位，必要时创建组。会话链接是该规则的一个入口。失败或取消不展开侧边栏，键盘焦点仍受后续用户交互保护。
